...
K-Beauty 클라이언트의 현재 구조에 맞춰 API를 연결한다. 이 서비스는 로그인 기능이 없고, 서버가 발급한 `X-Session-Token`으로 현재 세션을 저장/조회한다.
...
- 도메인 API 함수는 `src/shared/apis/{domain}.ts`에 작성한다.
- request/response DTO는 `src/shared/types/{domain}.ts`에 작성한다.
...
- `apiClient`의 baseURL, timeout, 세션 인터셉터
- 이미 존재하는 도메인 API 파일
- 이미 존재하는 DTO 타입
...
DTO 타입은 `src/shared/types/{domain}.ts`에 둔다.
...
도메인 API 파일은 `src/shared/apis/{domain}.ts`에 둔다.

```ts
import { apiClient } from '@/shared/apis/apiClient';
import type { CurrentSessionResponse } from '@/shared/types/session';

const SESSION_BASE_PATH = '/api/sessions';

const sessionApi = {
  create: () => apiClient.post<void>(SESSION_BASE_PATH),

  getCurrent: () => apiClient.get<CurrentSessionResponse>(`${SESSION_BASE_PATH}/current`),
};
...
세션 생성 API가 응답 헤더로 `X-Session-Token`을 내려주면 저장한다.
...
- 프론트에서 응답 헤더를 읽어야 하므로 서버 CORS에 `Access-Control-Expose-Headers: X-Session-Token`이 필요할 수 있다.
